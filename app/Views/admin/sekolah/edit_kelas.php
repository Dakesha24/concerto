...
<div class="pg-wrap">
    ...
    <nav class="pg-breadcrumb">
        ...
        <span class="pg-bc-sep"><i class="bi bi-chevron-right"></i></span>
        <span>Edit <?= esc($kelas['nama_kelas']) ?></span>
    </nav>

    <div class="pg-header">
        <div>
            <p class="pg-eyebrow">Kelola Kelas</p>
            <h1 class="pg-title">Edit Kelas <?= esc($kelas['nama_kelas']) ?></h1>
            <p class="pg-sub">Perbarui data kelas di <strong><?= esc($sekolah['nama_sekolah']) ?></strong>.</p>
        </div>
    ...
    <div class="info-bar mb-3">
        <i class="bi bi-building" style="color:#0051ba"></i>
        <strong><?= esc($sekolah['nama_sekolah']) ?></strong>
                ...
                <div class="col-12">
                    ...
                    <div class="quick-tags">
                        ...
                        for ($i = -1; $i <= 3; $i++):
                            $y = $cy + $i;
                            $ta = $y . '/' . ($y + 1);
                            $active = ($ta == $kelas['tahun_ajaran']) ? ' quick-tag--active' : '';
                        ?>
                            <button type="button" class="quick-tag<?= $active ?>" onclick="document.getElementById('tahun_ajaran').value='<?= $ta ?>'">
                                <?= $ta ?><?= $active ? ' <small>(saat ini)</small>' : '' ?>
                            </button>
                        <?php endfor; ?>
                    </div>
                </div>
...
.quick-tag:hover, .quick-tag--active { background:#0051ba; color:#fff; border-color:#0051ba; }
</style>
<style>
