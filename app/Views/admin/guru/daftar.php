...
<div class="pg-wrap">
    ...
    <div class="pg-header">
        <div>
            ...
            <h1 class="pg-title">Kelola Guru</h1>
            <p class="pg-sub">Daftar seluruh guru yang terdaftar dalam sistem CONCERTO.</p>
        </div>
        <a href="<?= base_url('admin/guru/tambah') ?>" class="btn-pg-action">
        ...
        <div class="row g-2 align-items-end">
            ...
            <div class="col-md-3">
                ...
                <select class="f-input" id="filterSekolah">
                    ...
                    <?php foreach (array_unique(array_column($guru, 'nama_sekolah')) as $s): if ($s): ?>
            ...
            <div class="col-md-3">
                ...
                <select class="f-input" id="filterMataPelajaran">
                    <option value="">Semua Mata Pelajaran</option>
                    <?php foreach (array_unique(array_column($guru, 'mata_pelajaran')) as $m): if ($m): ?>
            ...
            <div class="col-md-3">
                ...
                <input type="text" class="f-input" id="searchGuru" placeholder="Nama, email, NIP…">
            </div>
            <div class="col-md-1">
                ...
                <tbody>
                    <?php if (empty($guru)): ?>
                        <tr><td colspan="11">
                            <div class="tbl-empty">
                                ...
                                <a href="<?= base_url('admin/guru/tambah') ?>" class="btn-pg-action btn-pg-action--sm">
                                    <i class="bi bi-plus-lg"></i> Tambah Guru
                                </a>
                            </div>
                        </td></tr>
                    <?php else: ?>
                        <?php foreach ($guru as $i => $g): ?>
                            <tr data-status="<?= $g['status'] ?>" data-sekolah="<?= $g['nama_sekolah'] ?>" data-mapel="<?= $g['mata_pelajaran'] ?>">
                                <td class="tbl-num"><?= $i + 1 ?></td>
                                <td class="tbl-name"><?= esc($g['nama_lengkap'] ?? '—') ?></td>
                                <td class="tbl-muted"><?= esc($g['username']) ?></td>
                                <td class="tbl-muted"><?= esc($g['email']) ?></td>
                                <td class="tbl-muted"><?= esc($g['nip'] ?? '—') ?></td>
                                <td><span class="pg-badge"><?= esc($g['mata_pelajaran'] ?? '—') ?></span></td>
                                <td class="tbl-muted" style="font-size:.82rem"><?= esc($g['nama_sekolah'] ?? '—') ?></td>
                                <td>
                                    <span class="pg-badge <?= ($g['total_kelas'] ?? 0) > 0 ? '' : 'pg-badge--gray' ?>">
                                        <?= $g['total_kelas'] ?? 0 ?> Kelas
                                    </span>
                                </td>
                                <td>
                                ...
                                <td class="tbl-muted"><?= date('d/m/Y', strtotime($g['created_at'])) ?></td>
                                <td>
                                    <div class="act-group">
                                        <a href="<?= base_url('admin/guru/edit/' . $g['user_id']) ?>" class="act-btn" title="Edit"><i class="bi bi-pencil"></i></a>
                                        <?php if ($g['status'] == 'active'): ?>
                                            <a href="<?= base_url('admin/guru/hapus/' . $g['user_id']) ?>" class="act-btn act-btn--danger" title="Nonaktifkan"
                                               onclick="return confirm('Yakin ingin menonaktifkan guru ini?')"><i class="bi bi-person-x"></i></a>
                                        <?php else: ?>
                                            <a href="<?= base_url('admin/guru/restore/' . $g['user_id']) ?>" class="act-btn act-btn--green" title="Aktifkan"
                                               onclick="return confirm('Yakin ingin mengaktifkan guru ini?')"><i class="bi bi-person-check"></i></a>
                                        <?php endif; ?>
                                    </div>
                                </td>
                            </tr>
...
function filterTable() {
    ...
    document.querySelectorAll('#tableGuru tbody tr').forEach(row => {
        if (row.cells.length === 1) return;
        const match = (!s || row.dataset.status === s) &&
                      (!sk || row.dataset.sekolah === sk) &&
                      (!mp || row.dataset.mapel === mp) &&
...
function resetFilter() {
    ['filterStatus','filterSekolah','filterMataPelajaran'].forEach(id => document.getElementById(id).value = '');
